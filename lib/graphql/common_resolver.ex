...
defmodule Bonfire.API.GraphQL.CommonResolver do
  ...
  def fetch_context_edge(_, ids) do
    # {:ok, ptrs} = Needles.many(id: List.flatten(ids))
    Fields.new(Needles.list!(ids), &Map.get(&1, :id))
  end
  ...
  # FIXME
  if Bonfire.Common.Extend.module_exists?(Bonfire.Common.Repo.Delete) do
